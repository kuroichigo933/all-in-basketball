...
- The source-aware calibration schedule contains 79 visible boxes, one absent frame, and five occlusions. The calibration ranker localizes 76/79 visible boxes on both A/B exports and preserves one occlusion as prediction, but it was trained on these same sources. It has no black-ball footage and only one absent-ball negative.
...
- The source-aware combined calibration report reaches 0.962025-0.968153 ball F1 with precision and recall above 0.95 on both exports; candidate-oracle recall is 0.974684. Leave-one-clip-out candidate ranking was only 61/77, so the result is an intentionally overfit calibration milestone, not evidence of new-player, black-ball, or new-environment accuracy.
...
- The calibrated ranker uses candidate confidence, source, size, appearance, position, and pose-relative distances. It can rescue low-confidence true candidates and correct body locks, but a representative basketball-specific detector and substantially denser positive, negative, blur, black-ball, and occlusion labels are still required.
...
- The mixed calibration source covers crossover but is not holdout data. The pre-review ranker-aware result was live-three F1 0.836879-0.845070. Correcting five rapid blocks raises provisional live-three precision/recall to 0.800000/0.982456 and 0.802817/1.000000, but wider decoded review found additional stale sparse labels. No move release gate is unblocked.
- The rapid CSV blocks use contiguous 500 ms labels. The final four-second block contains eight labels but only five visible corrected centerline-transfer/bounce cycles, so its event granularity needs manual review before it can support a 95% repetition claim.
