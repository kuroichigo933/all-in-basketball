...
`BrowserBallDetector` is the model boundary. The primary pass uses the full-player pose crop. When it returns no learned ball and lower-body landmarks are reliable, the same detector receives one tighter wrist/hip/knee dribble-zone crop; its result is mapped back to full-frame coordinates. This adaptive pass raised learned-candidate frame coverage without adding a second call to already successful frames. `NEXT_PUBLIC_BASKETBALL_MODEL_URL` may select a MediaPipe-compatible one-class TFLite detector and `NEXT_PUBLIC_BASKETBALL_MODEL_LABELS` declares its labels. Initialization falls back to the generic EfficientDet sports-ball model on the same GPU/CPU delegate if the custom artifact fails, without changing the live or upload interfaces. A separate artifact verifier requires a versioned manifest, exact bytes and SHA-256, TFL3 container identifier, license/model-card evidence, exact MediaPipe runtime version, and evaluation/browser-smoke evidence before an artifact can be marked validated.

Move classification uses MediaPipe wrist depth normalized by hip width as an additional front-versus-shallow transfer cue. Depth never creates a move by itself: it gates an otherwise complete measured ball transfer, falls back to the 2D rule when z coordinates are unavailable, and abstains on ambiguous shallow crossover evidence rather than forcing a different class.
...
- `validation:ball-dataset` converts one or more calibration manifests into an ignored YOLO package for basketball-detector development. It refuses holdout export, rejects duplicate clip IDs, excludes occlusions, preserves true absent frames as empty negative labels, separates tiny partial positives for audit, and retains every source manifest. Its collection-readiness report blocks promotion when source-separated negatives, black/orange, player, lighting, or verified hard-negative coverage is missing; this report is not an accuracy metric.
