...
if (!existsSync(manifestPath)) { console.error(`Validation manifest not found: ${manifestPath}`); process.exitCode = 2; }
      ...
      for (const legRegionHipMargin of [0.15, 0.25, 0.35])
        for (const behindMaximumKneeSpreadHipWidths of [1.15, 1.25, 1.35])
          for (const poseTransferBallProximityHipWidths of [1.6, 2.2, 2.8])
            for (const poseTransferCrossoverMaximumWristDepthHipWidths of [-100, -7, -6.5, -6, -5.5])
              for (const lateralCrossoverMaximumWristDepthHipWidths of [-7, -6.5, -6, -5.5, 100])
                candidates.push({ ...DEFAULT_MOVE_DETECTION_CONFIG, lateralTravelHipWidths, hipBandHalfHeight, legRegionHipMargin,
                  behindMaximumKneeSpreadHipWidths, poseTransferBallProximityHipWidths,
                  poseTransferCrossoverMaximumWristDepthHipWidths, lateralCrossoverMaximumWristDepthHipWidths });
  const calibration = observationsDirectories.flatMap((observationsDirectory) => clips.map((clip) => {
  ...
  console.log(JSON.stringify({ calibrationClips: clips.length, calibrationRuns: observationsDirectories.length,
    ...
    tuningMoves, candidates: candidates.length, f1: best.f1, precision: best.precision, output }, null, 2));
}
