...
import { selectSplit, validateManifest, type AnalysisExport } from "../lib/motion/validation.ts";
...
if (!existsSync(manifestPath)) { console.error(`Validation manifest not found: ${manifestPath}`); process.exitCode = 2; }
  ...
  for (const lateralTravelHipWidths of [0.55, 0.7, 0.85]) for (const hipBandHalfHeight of [0.3, 0.45, 0.6]) for (const legRegionHipMargin of [0.15, 0.25, 0.35]) candidates.push({ ...DEFAULT_MOVE_DETECTION_CONFIG, lateralTravelHipWidths, hipBandHalfHeight, legRegionHipMargin });
  let best = { config: candidates[0], f1: -1, precision: -1 };
  for (const config of candidates) {
    const scores = clips.map((clip) => { const data = JSON.parse(readFileSync(resolve(dirname(manifestPath), clip.observations), "utf8")) as AnalysisExport; return evaluateDetections(clip.expected, detectMoves(trackBallContinuity(data.observations), config), manifest.toleranceMs); });
  ...
  console.log(JSON.stringify({ calibrationClips: clips.length, candidates: candidates.length, f1: best.f1, precision: best.precision, output }, null, 2));
}
