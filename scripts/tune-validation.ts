...
import { CONTROLLED_MOVE_NAMES, selectSplit, validateManifest, type AnalysisExport } from "../lib/motion/validation.ts";
...
if (!existsSync(manifestPath)) { console.error(`Validation manifest not found: ${manifestPath}`); process.exitCode = 2; }
  ...
  for (const config of candidates) {
    const scores = calibration.map(({ clip, observations }) => evaluateDetections(
      clip.expected.filter((label) => CONTROLLED_MOVE_NAMES.includes(label.move)),
      detectMoves(observations, config).filter((detection) => CONTROLLED_MOVE_NAMES.includes(detection.move)),
